...
const { server } = await import("../src/mcp-servers/keyword-tracker/server.js");
...
// ── MCP client wired to server via InMemoryTransport ─────────────────
let client: Client;
...
describe("get_rankings", () => {
  ...
  it("returns keywords sorted by position with correct fields", async () => {
    mockQuery.mockResolvedValue({ data: { rows: [
      row("office cleaning london",    7.8,  12, 200, 0.06),
      row("cleaning services near me", 3.2,  45, 520, 0.0865),
      row("commercial cleaner",        15.3,  5, 180, 0.0278),
    ]}});
    ...
    expect(d.rankings[0].keyword).toBe("cleaning services near me");
    ...
    expect(d.rankings[2].keyword).toBe("commercial cleaner");
...
describe("get_ranking_history", () => {
  ...
  it("returns history sorted by date ascending", async () => {
    mockQuery.mockResolvedValue({ data: { rows: [
      rowD("cleaning services near me", "2026-03-17", 3.2, 45, 520, 0.087),
      rowD("cleaning services near me", "2026-03-01", 5.0, 38, 400, 0.095),
      rowD("cleaning services near me", "2026-03-10", 3.5, 42, 450, 0.093),
    ]}});

    const res = await client.callTool({
      name: "get_ranking_history",
      arguments: { site_id: 1, keyword: "cleaning services near me", days: 30 },
    });
    const d = text(res.content);

    expect(res.isError).toBeFalsy();
    expect(d.keyword).toBe("cleaning services near me");
  ...
  it("passes keyword as dimension filter to GSC", async () => {
    ...
    await client.callTool({
      name: "get_ranking_history",
      arguments: { site_id: 1, keyword: "office cleaning", days: 14 },
    });

    const body = mockQuery.mock.calls[0][0].requestBody;
    expect(body.dimensions).toContain("date");
    expect(body.dimensionFilterGroups[0].filters[0].expression).toBe("office cleaning");
  });
  ...
  it("returns isError on API timeout", async () => {
    ...
    const res = await client.callTool({
      name: "get_ranking_history",
      arguments: { site_id: 1, keyword: "office cleaning" },
    });
  ...
  it("returns isError when keyword is missing", async () => {
    const res = await client.callTool({
      name: "get_ranking_history",
      arguments: { site_id: 1 }, // keyword is required
    });
  ...
  it("returns isError on API rate-limit (429)", async () => {
    ...
    const res = await client.callTool({
      name: "get_ranking_history",
      arguments: { site_id: 1, keyword: "cleaning" },
    });
...
describe("get_top_movers", () => {
  ...
  const CURRENT = [
    row("cleaning services near me", 3.2),
    row("office cleaning",           12.0),
    row("maid service",              8.0),
  ];
  const PREVIOUS = [
    row("cleaning services near me", 5.0),  // +1.8 improved
    row("office cleaning",           8.0),  // -4.0 declined
    row("maid service",              12.0), // +4.0 improved
  ];
  ...
  it("filters direction=down — only negative changes", async () => {
    ...
    expect(d.movers[0].keyword).toBe("office cleaning");
  });

  it("skips keywords absent from previous period", async () => {
    mockQuery
      .mockResolvedValueOnce({ data: { rows: [row("brand new keyword", 5), row("established kw", 3)] } })
    ...
    expect(keywords).not.toContain("brand new keyword");
...
describe("get_rank_velocity", () => {
  ...
  // Positions: [8,6,5,4,3] → slope=-1.2 → improving
  // Positions: [5,5.1,4.9] → slope≈-0.05 → stable
  // Positions: [3,5,7,9,11] → slope=+2.0  → declining
  const VELOCITY_ROWS = [
  ...
  it("sorts results by |velocity| descending", async () => {
    ...
    const res = await client.callTool({ name: "get_rank_velocity", arguments: { site_id: 1 } });
  ...
  it("reports 'stable' for a keyword with only 1 data point", async () => {
  ...
  it("returns isError for unknown site_id", async () => {
    process.env.GSC_OAUTH_SITE_99 = FAKE_SA;
    const res = await client.callTool({ name: "get_rank_velocity", arguments: { site_id: 99 } });
    delete process.env.GSC_OAUTH_SITE_99;
