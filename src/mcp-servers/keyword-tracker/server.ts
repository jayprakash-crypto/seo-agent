...
// ── MCP Server factory — one fresh instance per session ───────────────
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            site_id: {
              type: "number",
              description: "Site ID (1=https://lifecircle.in)",
            },
            keyword: {
              type: "string",
        ...
        case "get_ranking_history":
          return {
            content: [
              {
                type: "text",
                text: JSON.stringify(await getRankingHistory(a)),
              },
            ],
          };
        case "get_top_movers":
...
const keyword_tracker_path = express();
keyword_tracker_path.use(express.json());

const transports = new Map<string, StreamableHTTPServerTransport>();

keyword_tracker_path.post("/mcp", async (req, res) => {
...
keyword_tracker_path.get("/mcp", async (req, res) => {
  const sessionId = req.headers["mcp-session-id"] as string | undefined;
  ...
  // Keep-alive pings every 20s so Railway's proxy doesn't close the SSE connection
...
keyword_tracker_path.delete("/mcp", async (req, res) => {
  const sessionId = req.headers["mcp-session-id"] as string | undefined;
  const transport = sessionId ? transports.get(sessionId) : undefined;
...
keyword_tracker_path.get("/health", (_req, res) =>
  res.json({
    status: "ok",
    server: SERVER_NAME,
...
export { createServer };
export default keyword_tracker_path;